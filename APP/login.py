
###### Pequeno adendo, isso ainda é o esqueleto e ficará bem mais polido

from tkinter import *

janela = Tk()

janela.title("Login")

def click_button():
    login = ed1.get()
    senha = ed2.get()
    if login == "TesteDepoisMudar" and senha == "TesteDepoisMudar":
        lb3 = Label(janela, text="Bem vindo meu chapa")
        lb3.grid(row=4, column=1)
        lb3["bg"] = "yellowgreen"
        lb4.destroy()
    else:
        lb4 = Label(janela, text="ACESSO NEGADO!")
        lb4.grid(row=4, column=1)
        lb4["bg"] = "red"
        lb3.destroy()

lb1 = Label(janela, text="Login: ")
lb1.grid(row=0, column=0)

lb2 = Label(janela, text="senha: ")
lb1.grid(row=1, column=0)



ed1 = Entry(janela)
ed1.grid(row=0,column=1)

ed2 = Entry(janela, show='*')
ed2.grid(row=1,column=1)



bt1 = Button(janela, text="Confirmar", command=click_button)
bt1.grid(row=2, column=1)

janela.geometry("1200x2000+0+0")

janela.mainloop()
